...
const { data: word, error } = await useFetch(`/api/words/${route.params.slug}`)
...
const { busy, error: failure, run } = useForm(async () => {
  ...
  await navigateTo(`/w/${word.value!.slug}`)
})
</script>

<template>
  <article>
    <BreadCrumbs v-if="word" :trail="[{ label: word.headword, to: `/w/${word.slug}` }, { label: 'تعديل' }]" />
    <h1>تعديل الكلمة</h1>
    ...
    <form @submit.prevent="run">
      <fieldset :disabled="busy">
        ...
        <p><button type="submit">حفظ</button> <NuxtLink :to="`/w/${word?.slug}`">إلغاء</NuxtLink></p>
      </fieldset>
    </form>
  </article>
</template>
